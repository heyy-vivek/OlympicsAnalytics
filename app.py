...
if user_menu == 'Overall Analysis':
    ...
    fig = px.line(nations_over_time, x="Edition", y="Team")
...
if user_menu == 'Country-wise Analysis':
    ...
    country_list = df['Team'].dropna().unique().tolist()
    country_list.sort()

    # Insert the placeholder option at the beginning of the list
    country_list.insert(0, 'Select a Country')

    # Create the selectbox with the placeholder as the default value
    selected_country = st.sidebar.selectbox('Select a Country', country_list, index=0)

    # Check if a valid country is selected
    if selected_country != 'Select a Country':
        # Fetch the country-wise medal tally
        country_df = helper.yearwise_medal_tally(df, selected_country)
        fig = px.line(country_df, x="Year", y="Medal")
        st.title(selected_country + " Medal Tally over the years")
        st.plotly_chart(fig)

        # Fetch the country-event heatmap
        ...
        # Fetch the top 10 athletes of the selected country
        st.title("Top 10 athletes of " + selected_country)
        top10_df = helper.most_successful_countrywise(df, selected_country)
        st.table(top10_df)
    else:
        st.write("Please select a country to see the analysis.")
